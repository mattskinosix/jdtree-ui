<template>
  ...
  <div :style="{ transform }" ref="myid">
    <div :style="{ width: divWidth + this.store.width + '%',height: divWidth + this.store.width + '%' , 'transform': 'scale('+Math.pow((divWidth + this.store.width)*1.05/100,-1)+') translate(-'+this.store.width*  0.525+'%) translateY(-'+this.store.width*0.5+'%)'}, 'overflow-y': 'hidden'"> 
      <TreeNode 
        :root=true
